...
server/
├── app/
│   ├── main.py                    # FastAPI 앱 + 라이프사이클 이벤트
│   ├── database.py                # DatabaseManager (Redis + MongoDB)
│   ├── core_config.py             # 환경 변수 설정
│   ├── dependencies.py            # 의존성 주입
│   ├── routers/
│   │   └── server.py              # API 엔드포인트 (upload, result, stats)
│   └── services/
│       └── inference/             # 추론 서비스 (5가지 탐지 모델)
│           ├── cnn_detector.py
│           ├── deepface_detector.py
│           ├── face_recognition_detector.py
│           ├── insightface_detector.py
│           └── ensemble_detector.py
├── models/                        # AI 모델 (미사용 legacy)
├── dataset/                       # 학습 데이터셋 (선택)
├── .env                           # 환경 변수 설정 파일
...
└─────────────────┬───────────────────────┘
                  │
┌─────────────────▼───────────────────────┐
│    Router Layer (routers/server.py)     │
...
│  - Redis Cache    (24h TTL, ~10ms)     │
│  - MongoDB Store  (영구, 통계)           │
│  - Memory Fallback (DB 없을 때)          │
└─────────────────┬───────────────────────┘
                  │
┌─────────────────▼───────────────────────┐
│   Service Layer (services/inference/)   │
│  - Ensemble Detector (89.4% 신뢰도)     │
│  - CNN / DeepFace / FaceRec / Insight  │
└─────────────────────────────────────────┘
```
